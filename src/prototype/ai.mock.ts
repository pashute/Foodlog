...
// in ai.ts — callers never need to know which branch they're on.
//
// The "fixed" key below is the exact output of Diary.tsx's buildFixString()
...
//
// analyze() throws for any text not in cannedAnalysis — Diary.tsx shows
...
const cannedAnalysis = {
  ...
  // Mark placement (Aug 19 original-record rework, diaryEntry.ts): a field
...
// recordPattern(), called from diaryEntry.ts's save flow via ai.ts's
// learnFromRecord()) — same session-only persistence style as sheet.mock.ts.
let patternContext = {
...
// Whitespace-normalized lookup: MealTextFormat (sheet.ts) inserts real
