...
// POST /log                the browser-side sheet.mock.js (a separate
...
import * as sheetMock from '../sheet.mock.js'
...
// Port comes from the same config URL sheet.mock.js reads (urls.sheet-mock-base) —
// one source of truth instead of a separately-hardcoded number here.
const PORT = Number(new URL(getByKey(KEYS.keyUrlSheetMock)).port)
...
function rowsToHtml(sheet) {
  if (!sheet.rows.length) {
    return '<tr><td colspan="6">(no entries yet)</td></tr>'
  }
  return sheet.rows
    .map(
      (r) =>
        `<tr><td>${escapeHtml(r.date)}</td><td>${escapeHtml(r.dow)}</td><td>${escapeHtml(r.time)}</td>` +
        `<td>${escapeHtml(r.carbs)}</td><td>${escapeHtml(r.status)}</td><td>${escapeHtml(r.meal)}</td></tr>`
    )
    .join('\n')
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  start()
  console.log(`Mock sheet server running at ${getByKey(KEYS.keyUrlSheetMock)}/Foodlog.mock.html`)
}
