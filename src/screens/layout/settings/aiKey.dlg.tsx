...
export const privacyStatementUrl = appConstants.urls.googlePrivacy
export const aiStudioUrl = appConstants.urls.googleAiStudio

export default function AiKeyDlg({ visible, onSave = () => {}, onClose = () => {} }) {
  const [value, setValue] = useState('')
  const [showInvalid, setShowInvalid] = useState(false)
  const [showScreenshot, setShowScreenshot] = useState(false)
  // Live validity indicator for the field being typed, independent of the
  // Settings screen's own AI-key LED (that one reflects the last SAVED key,
  // not whatever is currently typed here). 'off' = no text yet.
  const [indicator, setIndicator] = useState('off')

  const valid = indicator === 'valid'

  // Aug 19 batch: indicator off -> validate -> (Save stays disabled unless
  // valid; an invalid attempt below never reaches storage, i.e. is never
  // "kept" as the key) -> enable Save if valid.
  const handleChangeText = (text) => {
    ...
    if (!text) {
      ...
      setIndicator(keyStatus(text) === 'ok' ? 'valid' : 'invalid')
    }
  }
  ...
  const handleSave = () => {
    ...
    setValue('')
    setShowInvalid(false)
    setIndicator('off') // after saved: re-disable Save (empty field is never valid)
  }
      ...
      <View style={styles.overlay}>
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          ...
          <TextInput
            ...
            autoCorrect={false}
          />
          {indicator === 'valid' && <Text style={styles.indicatorValid}>✓ looks valid</Text>}
          {indicator === 'invalid' && <Text style={styles.indicatorInvalid}>✗ invalid format</Text>}
...
const styles = StyleSheet.create({
  ...
  indicatorValid: { color: '#4ade80', fontSize: 13, marginBottom: 10 },
  indicatorInvalid: { color: '#fbbf24', fontSize: 13, marginBottom: 10 },
  saveBtn: {
