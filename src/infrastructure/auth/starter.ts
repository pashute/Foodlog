...
// plain `node --test`/cucumber can't load a .tsx file at all (unknown file
// extension, not even a parse issue), so auth.ts and feature step defs
...
// on either of them, so importing it here doesn't risk the auth.ts <->
// starter.ts circular import that duplicating the literal was working
// around).
