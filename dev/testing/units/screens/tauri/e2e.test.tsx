...
import { appConstants, configuration } from '../../../../../src/infrastructure/config/config.ts'
import { initialize } from '../../../../../src/infrastructure/storage/storage.ts'
...
test('settings page on app start shows all expected texts, matching config', () => {
  ...
  expect(screen.getAllByText(appConstants.appName).length).toBeGreaterThanOrEqual(1)
  expect(screen.getByText(`v${appConstants.appVersion}`)).toBeInTheDocument()
  expect(screen.getByText('Login with Google')).toBeInTheDocument()

  expect(screen.getByText('Theme')).toBeInTheDocument()
  expect(screen.getByText(configuration.app.theme === 'dark' ? 'Dark' : 'Light')).toBeInTheDocument()

  expect(screen.getByText('AI Key Missing')).toBeInTheDocument()
  expect(screen.getByText('Start AI')).toBeInTheDocument()

  expect(screen.getAllByText(configuration.sheets.sheetName).length).toBeGreaterThanOrEqual(1)
...
// auth.ts dynamically import()s starter.ts/oauth.mock.ts on every step, so
