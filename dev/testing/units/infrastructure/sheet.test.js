// Filename sheet.test.js  Version 0.2.2
...
import {
  ...
  MealTextRegex,
  isValidMealText,
} from '../../../../src/infrastructure/sheet/sheet.js'
...
// as ai.js's real Gemini call: written, not exercised, per instruction).
// Skipped, not deleted, so it's ready to run once config.data.js's stage
// flips to non-'prototype' against a real signed-in session with a stored
// authToken. existsOrCreate()/log()/link() are async in the real branch
// (Promise), unlike the sync prototype branch — see sheet.js's note on that.
test('sheet module (production, real Google Sheet) — needs live Google auth', { skip: 'requires a real signed-in Google session; run manually against production config' }, async () => {
  const { existsOrCreate, log, link } = await import('../../../../src/infrastructure/sheet/sheet.js')
