...
Feature: screens/layout/header
  ...
  Scenario: Header layout is correct
    ...
    And the version has lowercase v followed by the version string from the app configuration with a smaller font
  ...
  Scenario: Header has configured content
    ...
    And lowercase v followed by the version string from the app configuration is shown next to the app name
