...
// Note: 'the app is open' is defined in screens/layout/header/step_definitions/header.steps.ts
...
// ---------------- @auth.scope-popup ----------------
// Note: doesn't use starter.ts's `_queueTestResponse` shortcut here, since
// this scenario is specifically about the pending/finish behavior itself
// (`isPending()` is a proxy for "is shown" — see starter.ts).

When('the user presses Continue Login with Google', async function () {
  this.starter = await import('../../../../../src/infrastructure/auth/starter.ts')
...
Then('verify that its a refresh token \\(access_type=offline and prompt=consent)', function () {
  ...
  // (access_type/prompt) — those belong to the real oauth.tauri/android.ts
...
Then('the app shows a message that login failed', function () {
  throw new Error('Not implemented yet') // UI-layer, not auth.ts's concern
})
...
Then('all app data is cleared:', async function (table) {
  // auth.logout() now clears oauth.mock.ts's in-memory session state plus
...
// ---------------- @auth.web.login / @auth.web.silent / @auth.web.logout ----------------
// oauth.web.ts's real branch needs window.google (Google Identity
// Services) and a rendered consent popup — a real browser, not this
// cucumber/node tier. Left "Not implemented yet", not faked, same as the
// other real (non-prototype) platform modules above.

Given('the user is on Expo web, not in prototype mode', function () {
  throw new Error('Not implemented yet')
})

When('the user completes the Google Identity Services consent popup', function () {
  throw new Error('Not implemented yet')
})

Then('the result carries an accessToken and scope {string}', function () {
  throw new Error('Not implemented yet')
})

Then('the result has no refreshToken', function () {
  throw new Error('Not implemented yet')
})

Given('the user is on Expo web with an active Google browser session', function () {
  throw new Error('Not implemented yet')
})

When('the app requests a token with prompt {string} \\(no popup)', function () {
  throw new Error('Not implemented yet')
})

Then('a fresh access token is returned with no user interaction', function () {
  throw new Error('Not implemented yet')
})

Given('the user is logged in on Expo web', function () {
  throw new Error('Not implemented yet')
})

When('the user logs out of the web session', function () {
  throw new Error('Not implemented yet')
})

Then('the access token is revoked via Google Identity Services', function () {
  throw new Error('Not implemented yet')
})
